...
func (s *gpuPoolService) reconnectWindow(ctx context.Context, pool *models.GPUPool, userID string) (eligible bool, until *time.Time, skipStartup bool) {
	if pool == nil || userID == "" {
	...
	untilTime := stopped.StoppedAt.Add(s.reconnectCooldownFor(ctx, pool.ServiceTag))
...
func (s *gpuPoolService) chargeSessionStartup(ctx context.Context, serviceTag, userID string) error {
	...
	if eligible, _, skip := s.reconnectWindow(ctx, pool, userID); skip && eligible {
...
// sessionCreditBreakdown splits GPU-pool session counters for status/API display.
// CreditsStartupCharged may be 20 on reconnect without debiting the wallet again;
// only count startup in gpu-time split when it was actually added to CreditsCharged.
func sessionCreditBreakdown(sess *models.GPUPoolSession) (startupBooked, startupDebited, gpuTime int) {
	if sess == nil {
		return 0, 0, 0
	}
	startupBooked = sess.CreditsStartupCharged
	if startupBooked > 0 && sess.CreditsCharged >= startupBooked {
		startupDebited = startupBooked
	}
	gpuTime = sess.CreditsCharged - startupDebited
	if gpuTime < 0 {
		gpuTime = 0
	}
	return startupBooked, startupDebited, gpuTime
}
...
func (s *gpuPoolService) checkStartCredits(ctx context.Context, userID, serviceTag string) error {
	...
	if eligible, _, _ := s.reconnectWindow(ctx, pool, userID); eligible {
